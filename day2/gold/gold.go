...
func gamePower(games []game) uint64 {
	mingame := game{0,0,0}
	for _,play := range games {
		mingame = game{
			max(mingame.red, play.red),
			max(mingame.green, play.green),
			max(mingame.blue, play.blue),
		}
	}
	return uint64(mingame.red)*uint64(mingame.green)*uint64(mingame.blue)
}
...
func main() {
	...
	fmt.Println(allGames,sum,powersum)
